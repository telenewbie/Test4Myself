...
static void UpdateCoherenceSpectraSSE2(int mult,
  ...
  for (i = 0; i + 3 < PART_LEN1; i += 4) {
...

    {
	 ...
	 //add by duanxiang
	{
		const __m128 vec_3210 = _mm_loadu_ps(&coherence_state->sex[i][0]);
		const __m128 vec_7654 = _mm_loadu_ps(&coherence_state->sex[i + 2][0]);
		__m128 vec_a =
			_mm_shuffle_ps(vec_3210, vec_7654, _MM_SHUFFLE(2, 0, 2, 0));
		__m128 vec_b =
			_mm_shuffle_ps(vec_3210, vec_7654, _MM_SHUFFLE(3, 1, 3, 1));
		__m128 vec_efwxfw0011 = _mm_mul_ps(vec_efw0, vec_xfw0);
		__m128 vec_efwxfw0110 = _mm_mul_ps(vec_efw0, vec_xfw1);
		vec_a = _mm_mul_ps(vec_a, vec_GCoh0);
		vec_b = _mm_mul_ps(vec_b, vec_GCoh0);
		vec_efwxfw0011 =
			_mm_add_ps(vec_efwxfw0011, _mm_mul_ps(vec_efw1, vec_xfw1));
		vec_efwxfw0110 =
			_mm_sub_ps(vec_efwxfw0110, _mm_mul_ps(vec_efw1, vec_xfw0));
		vec_a = _mm_add_ps(vec_a, _mm_mul_ps(vec_efwxfw0011, vec_GCoh1));
		vec_b = _mm_add_ps(vec_b, _mm_mul_ps(vec_efwxfw0110, vec_GCoh1));
		_mm_storeu_ps(&coherence_state->sex[i][0], _mm_unpacklo_ps(vec_a, vec_b));
		_mm_storeu_ps(&coherence_state->sex[i + 2][0],
			_mm_unpackhi_ps(vec_a, vec_b));
	}
...
static void ComputeCoherenceSSE2(const CoherenceState* coherence_state,
...

  {
    ...
    for (i = 0; i + 3 < PART_LEN1; i += 4) {
	  ...
	  const __m128 vec_sesx =
		  _mm_add_ps(vec_1eminus10, _mm_mul_ps(vec_se, vec_sx));//add by duanxiang
	  ...
	  const __m128 vec_sex_3210 = _mm_loadu_ps(&coherence_state->sex[i][0]);//add by duanxiang
	  const __m128 vec_sex_7654 = _mm_loadu_ps(&coherence_state->sex[i + 2][0]);//add by duanxiang
	  ...
	  const __m128 vec_sex_0 =
		  _mm_shuffle_ps(vec_sex_3210, vec_sex_7654, _MM_SHUFFLE(2, 0, 2, 0));//add by duanxiang
	  const __m128 vec_sex_1 =
		  _mm_shuffle_ps(vec_sex_3210, vec_sex_7654, _MM_SHUFFLE(3, 1, 3, 1));//add by duanxiang
      __m128 vec_cohde = _mm_mul_ps(vec_sde_0, vec_sde_0);
      __m128 vec_cohxd = _mm_mul_ps(vec_sxd_0, vec_sxd_0);
	  __m128 vec_cohex = _mm_mul_ps(vec_sex_0, vec_sex_0);//add by duanxiang
	  ...
	  vec_cohex = _mm_add_ps(vec_cohex, _mm_mul_ps(vec_sex_1, vec_sex_1));//add by duanxiang
	  vec_cohex = _mm_div_ps(vec_cohex, vec_sesx);//add by duanxiang
      _mm_storeu_ps(&cohde[i], vec_cohde);
      _mm_storeu_ps(&cohxd[i], vec_cohxd);
	  _mm_storeu_ps(&cohex[i], vec_cohex);//add by duanxiang
    }
