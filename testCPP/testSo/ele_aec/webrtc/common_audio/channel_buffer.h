...
class  IFChannelBuffer {
 public:
	 __declspec(dllexport)  IFChannelBuffer(size_t num_frames, size_t num_channels, size_t num_bands = 1);
	 __declspec(dllexport) ~IFChannelBuffer();

	 __declspec(dllexport)  ChannelBuffer<int16_t>* ibuf();
	 __declspec(dllexport)  ChannelBuffer<float>* fbuf();
	 __declspec(dllexport)  const ChannelBuffer<int16_t>* ibuf_const() const;
	 __declspec(dllexport) const ChannelBuffer<float>* fbuf_const() const;

	 __declspec(dllexport) size_t num_frames() const { return ibuf_.num_frames(); }
	 __declspec(dllexport) size_t num_frames_per_band() const { return ibuf_.num_frames_per_band(); }
	 __declspec(dllexport) size_t num_channels() const {
    return ivalid_ ? ibuf_.num_channels() : fbuf_.num_channels();
  }
	 __declspec(dllexport) void set_num_channels(size_t num_channels) {
    ibuf_.set_num_channels(num_channels);
    fbuf_.set_num_channels(num_channels);
  }
	 __declspec(dllexport)  size_t num_bands() const { return ibuf_.num_bands(); }
