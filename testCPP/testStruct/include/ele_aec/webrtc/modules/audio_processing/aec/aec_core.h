...
#ifdef _WIN32
#define ELEVOC_API_EXPORT
#else
#define ELEVOC_API_EXPORT __attribute__((visibility("default")))
#endif  
...
#define MaxPsdwDiffSize 5
#define PathchangeResetDuritionTime 75// the adaptive filter convergence time
#define OutputBufferSize 6
#define HD_ORDER 6
#define NormalizedTime 6
#define NLPHistogramSize 20
...
static const int kNormalNumPartitions = 20;
...
typedef struct CoherenceState {
	...
	complex_t sex[PART_LEN1];  // add by duanxiang
...
typedef struct PostFilter {
    ...
    float n_min_temp;
    float p_noise;
    float e_power_beforeNlp;
    float d_power_beforeNlp;
    float suppress_percent[10];
	float hNlDeAvg;
	float hNlXdAvg;
    float hNlExAvg;
	CoherenceState postcoh;
} PostFilter;

typedef struct ResidualEchoSupperssion {
	float Sx[PART_LEN1], Sd[PART_LEN1], Se[PART_LEN1];
	float eta_2term[PART_LEN1];
	float eta_min;
	float Sres[PART_LEN1]; // variance residual echo
	float eta_n2term[PART_LEN1];
	float GH1_n0[PART_LEN1];
	float GH1_n[PART_LEN1];
	float Sed[PART_LEN1];
	float xi_MECC;
	//reverb and dtd used
	float rem[2][PART_LEN1];
	float sigma_m2[PART_LEN1];
	float alpha_s[PART_LEN1];
	float alpha_err[PART_LEN1];
	float alpha_res[PART_LEN1];
	float alpha_dtd[PART_LEN1];
	float alpha_dts[PART_LEN1];
	//harmonic distorition
	float est_echo_psd_lowband[PART_LEN1];
	float est_echo_psd_alpha;

	float XFm[2][NormalizedTime*PART_LEN1];
	float WFb[2][kNormalNumPartitions*PART_LEN1];
	// nlp
	float Se2[PART_LEN1], Sd2[PART_LEN1];
	float Sed2[2][PART_LEN1];

} ResidualEchoSupperssion;

typedef struct PathChangeDetection {
	// path change detection
	float last_max_sumpsdw_partition;
	float max_sumpsdw_diff_vector[MaxPsdwDiffSize];
	int pathchange_reset_durition;
	float div_psdw_th;
} PathChangeDetection;
...
struct AecCore {
  ...
  float linear_output_buffer[NUM_HIGH_BANDS_MAX + 1][2 * PART_LEN];
  size_t output_buffer_size;
  size_t linear_output_buffer_size;
  ...
  float xPow[PART_LEN1]; //far .
  float dPow[PART_LEN1]; //near .
  float dMinPow[PART_LEN1];//near min pow .
  float dInitMinPow[PART_LEN1];//near init pow .
  float* noisePow;

  float xfBuf[2][kExtendedNumPartitions * PART_LEN1];  // farend fft buffer  32 * 65
  float wfBuf[2][kExtendedNumPartitions * PART_LEN1];  // filter fft		32 * 65
  // Farend windowed fft buffer.
  complex_t xfwBuf[kExtendedNumPartitions * PART_LEN1]; // farend windowed fft buffer .
  ...
  int cur_divergence;
  int first_move_elements;

  int near_vad_res;
  int far_vad_res;
    
  PostFilter post_filter;// add by duanxiang
  ResidualEchoSupperssion residEchoSupp;
  PathChangeDetection  pcd;
  float hnlFbIdx_L;
  float hnlFbIdx_H;

  float energyHband;
  int nlpHistogram[NLPHistogramSize];
  float energyFinal;

  float output_nlpblock_buffer[NUM_HIGH_BANDS_MAX + 1][OutputBufferSize*PART_LEN];
  float comfortNoise_factor;
...
ELEVOC_API_EXPORT void nlpSuppress(AecCore* self, size_t *seed, float *ns_audio, float *eBuf,float *outBuf, float output[NUM_HIGH_BANDS_MAX + 1][PART_LEN], float *hNl, float *noisePow, float *fft, float efw[2][PART_LEN1], float previous_nearend_block[NUM_HIGH_BANDS_MAX + 1][PART_LEN]);
}  // namespace webrtc
