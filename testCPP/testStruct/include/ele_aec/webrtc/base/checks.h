...
class  FatalMessage {
 public:
  __declspec(dllexport) FatalMessage(const char* file, int line);
  // Used for RTC_CHECK_EQ(), etc. Takes ownership of the given string.
  __declspec(dllexport)  FatalMessage(const char* file, int line, std::string* result);
  __declspec(dllexport)  NO_RETURN ~FatalMessage();
