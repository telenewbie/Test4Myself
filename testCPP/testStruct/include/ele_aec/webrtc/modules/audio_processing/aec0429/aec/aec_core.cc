...
static void Overdrive(float overdrive_scaling,
  ...
  for (int i = 0; i < PART_LEN1; ++i) {
	...
	hNl[i] = powf(hNl[i], overdrive * WebRtcAec_overDriveCurve[i]);
  }
}
		...
		if (aec->stNearState == 1) {
			...
			if (clip_rate && hNlExAvg > 0.132f) {//回声破音
				...
				aec->nlp_process = 30000;//2min
										 //printf(" clip start time = %d, clip_rate = %f \n",overdrivetime,clip_rate);
			}
...
static void EchoSuppression(const OouraFft& ooura_fft,
 ...
 /* aec->data_dumper->DumpRaw("aec_cohde", PART_LEN1, cohde);
  ...
  float efw_raw[2][PART_LEN1];
  memcpy(efw_raw,efw,sizeof(float)*2*PART_LEN1);

  for (int p = 0; p < 2; p++) {
	  float output_nlp[NUM_HIGH_BANDS_MAX + 1][PART_LEN];
	  memcpy(efw,efw_raw,sizeof(float)*2*PART_LEN1);
	  if (p == 1) {
		  memcpy(hNl,aec->residEchoSupp.GH1_n,sizeof(float)*PART_LEN1);
	  }
	...
	//   ComfortNoise(aec->num_bands > 1, &aec->seed, efw, comfortNoiseHband,
	  ...
	  for (i = 0; i < PART_LEN; i++) {
		  output_nlp[0][i] = (fft[i] * WebRtcAec_sqrtHanning[i] +
			  aec->outBuf[p][i] * WebRtcAec_sqrtHanning[PART_LEN - i]);

		  // Saturate output to keep it in the allowed range.
		  output_nlp[0][i] = WEBRTC_SPL_SAT(WEBRTC_SPL_WORD16_MAX, output_nlp[0][i],
			  WEBRTC_SPL_WORD16_MIN);
	  }
	  memcpy(aec->outBuf[p], &fft[PART_LEN], PART_LEN * sizeof(float));
	  // For H band
	  if (aec->num_bands > 1) {
		  // H band gain
	  ...
	  for (i = 0; i < aec->num_bands; ++i) {
		  memmove(&aec->output_nlpblock_buffer[p][i][PART_LEN], &aec->output_nlpblock_buffer[p][i][0],
			  sizeof(float)*(OutputBufferSize - 1)*PART_LEN);
		  memcpy(&aec->output_nlpblock_buffer[p][i][0], &output_nlp[i][0], sizeof(float) * PART_LEN);
	  }
  }

  if(aec->pcd.pathchange_reset_durition>0) {
	  for (i = 0; i < aec->num_bands; ++i) {
		   memcpy(&output[i][0], &aec->output_nlpblock_buffer[0][i][(OutputBufferSize - 1)*PART_LEN], sizeof(float) * PART_LEN);
	  }
  }
  else {
	  for (i = 0; i < aec->num_bands; ++i) {
		  memcpy(&output[i][0], &aec->output_nlpblock_buffer[1][i][(OutputBufferSize - 1)*PART_LEN], sizeof(float) * PART_LEN);
	  }
  }
...
int WebRtcAec_InitAec(AecCore* aec, int sampFreq) {
 ...
 // memset(aec->Nearbuffer,0,sizeof(aec->Nearbuffer));
  ...
  memset(aec->outBuf, 0, sizeof(float) * PART_LEN*2);
